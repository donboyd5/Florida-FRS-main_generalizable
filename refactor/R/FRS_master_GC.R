...
print(
  "This is a full run using GC version, so creating model parameters and frs_data environments from scratch..."
)
...
print(
  "sourcing FRS_benefit_model_helper_functions.R and FRS_benefit_model_get_benefit_data_function.R..."
)
bm_env <- new.env()
...
source(
  fs::path(rdir, "FRS_benefit_model_get_benefit_data_function_GC_s.R"),
  local = bm_env
) # only creates functions - no live code, also into the bm_env environment
...
source(
  fs::path(rdir, "FRS_funding_model_functions_loop_only.R"),
  local = fm_env
) # only creates function - no live code
...
system.time(source(
  fs::path(rdir, "FRS_workforce_model_get_saved_data_s.R"),
  local = wf_data_env
)) # < 1 sec -- only gets saved data - no functions
...
params$wf_data_list <- mget(
  paste0(params$class_names_no_drop_frs_, "_wf_data"),
  envir = wf_data_env
) # does not waste memory because R is copy on modify
params$entrant_profile_table_list <- frs_data_env$entrant_profile_table_list # previously created

params$salary_headcount_table_list <- mget(
  paste0(params$class_names_no_drop_frs_, "_salary_headcount_table"),
  envir = frs_data_env
)

params$mort_table_list <- mget(
  paste0(params$class_names_no_drop_frs_, "_mort_table"),
  envir = frs_data_env
)
params$mort_retire_table_list <- mget(
  paste0(params$class_names_no_drop_frs_, "_mort_retire_table"),
  envir = frs_data_env
)

params$separation_rate_table_list <- mget(
  paste0(params$class_names_no_drop_frs_, "_separation_rate_table"),
  envir = frs_data_env
)

params$normal_retire_rate_tier_1_table_list <- mget(
  paste0(params$class_names_no_drop_frs_, "_normal_retire_rate_tier_1_table"),
  envir = frs_data_env
) # defined in benefit model actions
params$normal_retire_rate_tier_2_table_list <- mget(
  paste0(params$class_names_no_drop_frs_, "_normal_retire_rate_tier_2_table"),
  envir = frs_data_env
) # defined in benefit model actions

params$early_retire_rate_tier_1_table_list <- mget(
  paste0(params$class_names_no_drop_frs_, "_early_retire_rate_tier_1_table"),
  envir = frs_data_env
) # defined in benefit model actions
params$early_retire_rate_tier_2_table_list <- mget(
  paste0(params$class_names_no_drop_frs_, "_early_retire_rate_tier_2_table"),
  envir = frs_data_env
) # defined in benefit model actions

# get funding and amortization data --------------------------------------------
params$funding_list <- fm_env$get_all_classes_funding_list(
  params$init_funding_data,
  params
)
params$current_amort_layers_table <- fm_env$get_current_amort_layers_summary_table(
  params$current_amort_layers_table_
)
